...
# data_version <- "c2-century_obs_09_25"
data_version <- "c2-century_pred_09_25"
...
covariables <- c(
  ...
  # 'mb_water_39y_accumulated', 
  # 'mb_water_39y_recurrence', 
  ...
  # Mapbiomas Water Collection 3 - dynamic
  'mb_water_recurrence_dynamic',
  
  # Mapbiomas Degradation (Beta) (SUM(30, 60, 90, 150, 300m))  - static
  'mb_summed_edges', 
...
# Filtra o conjunto de dados para incluir apenas as amostras com 'loss' = 1
...
for (i in seq(along.with = random_seeds)) {
  ...
  ## Criação dos folds para validação cruzada usando apenas o subconjunto de validação
  # O índice do conjunto de dados de validação é criado a partir do dataset original,
  # mas contendo apenas as observações onde `loss` é igual a 1.
  validation_folds_indices <- createFolds(
    validation_data$estoque,
    k = 10,
    list = TRUE,
    returnTrain = FALSE
  )
  
  # A seguir, mapeamos esses índices de volta para o `original_data` para que `caret`
  # possa identificar corretamente quais amostras usar em cada fold.
  validation_folds_original_indices <- lapply(validation_folds_indices, function(fold_indices) {
    which(original_data$dataset_id %in% validation_data$dataset_id[fold_indices])
  })
  ...
  cv_control_object <- trainControl(
    ...
    # Usa os índices personalizados para a validação cruzada
    indexOut = validation_folds_original_indices
  )
